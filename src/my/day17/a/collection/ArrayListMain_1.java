...
public class ArrayListMain_1 {
	...
	public static void main(String[] args) {
		...
		/*
		 Array(배열)와 Collection(구조체)의 차이점 
		 1. Array는 크기가 한정되어져 있다. 한번 초기값을 넣으면 그 이상 값을 넣을 수 없다.
		 	Collection은 크기가 무한정이다.
		 
		 2.Array는 배열에 선언되어진 동일한 타입의 데이터만 입력가능
		 Collection은 입력되어지는 데이터가 객체(Object)이기만 하면 어떤것이든지 입력가능함.
		...
		*/
		...
		nameArr[0] = "이순신";		//String
		//nameArr[1] = 1234.5;		//double이므로 오류
		//nameArr[2] = 98;			//int이므로 오류
		//nameArr[3] = new Member();	//Member이므로 오류 클래스를 넣을 수 없다
		...
		*/
		...
		nameList.add(1234.5);	//double --> Double. Wrapper클래스로 오토박싱
		
		nameList.add(new Integer(98));
		nameList.add(98);	//int --> Integer. Wrapper 클래스로 오토박싱
		...
		///////////////////////////////////////////////////
		// === Generic ===
		/* 원래 Collection은 객체이기만 하면 모두 입력이 가능. 하지만 
		 입력은 모두 되지만 꺼내와서 사용할때 문제가 발생한다. 수많은 데이터 중에서 특정 데이터타입을 가지는 데이터를 꺼내오기는 귀찮은 작업을 해야만 한다.
		 따라서 배열처럼 특정한 타입의 데이터만 입력하도록 만든 것이 Generic이다
		...
		//또는
		List<String> strList = new ArrayList<>();	//JDK 8부터 가능.
		...
		//mbrList.add(10);		//오류 발생. mbrList에는 Generic으로 선언된 Member 객체만 들어온다
