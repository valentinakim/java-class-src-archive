...
public class Gujikja {
	...
	String passwd;
	String name;
	String jubun; 	//주민번호 앞의 7가지 까지만
					// 예: "9501151"  남자		0105103 남자
					// 예: "9501152" 여자		0105104 여자
	
	
	static int count;	// Gujikja 객체(인스턴스)의 개수를 알아오기 위한 용도.
	
	//method 생성(==> field(속성)를 어떤 방식으로 처리하고자 할때 메소드를 생성) ex)주민번호로 성별을 알아보고자 할때 메소드 생성
	...
	int getAge() {
		...
		String sgenderNum = jubun.substring(6);	//"1" 또는 "2" 또는  "3" 또는  "4"
		
		String sbirthYear = jubun.substring(0, 2);
		// "95" 또는 "01"
		
		int nbirthYear = Integer.parseInt(sbirthYear);	// 95 또는 1

		if("1".equals(sgenderNum)||"2".equals(sgenderNum)) { //구직자가 1900년대생
