...
// 아래는 my.day18.c.ENUM.EntranceFee_2 클래스를 enum을 사용하여 변경한 것이다.
...
public enum EntranceFee_2 {
	...
	CHILD(0), TEENAGER(150), ADULT(300), OLD(100);	//끝에 반드시 ; 붙인다.
	// !!! 사실은 열거형 상수 CHILD, TEENAGER, ADULT, OLD 하나 하나가 EntranceFee_2 객체라는 것이다.!!!
	
	
	private final int FEE;
	// enum(열거형)의 인스턴스 변수는 반드시 final 이어야 한다는 규칙은 없지만 
    // fee는 열거형 상수값을 저장하기 위한 용도이므로 final 을 붙인 것이다. fianl이 붙은 필드는 관례상 대문자를 사용한다. 
	
	EntranceFee_2(int fee) {	//파라미터 생성자
