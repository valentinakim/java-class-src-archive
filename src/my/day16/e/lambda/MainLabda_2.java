...
public class MainLabda_2 {

	public static void main(String[] args) {
		
		System.out.println("=== 람다식(Lambda)을 사용하지 않은 것 ===");
		
		InterAreaFuntional_1 obj1 = new InterAreaFuntional_1() {
		...
		System.out.println("가로 10.5, 세로 5.5인 면적 : "+areaSize1);
		...
		InterAreaFuntional_1 obj2 = (x,y) -> {return x*y;};
		double areaSize2 = obj2.area(10.5, 5.5);
		
		System.out.println("가로 10.5, 세로 5.5인 면적 : "+areaSize2);
	
		System.out.println("\n~~~~~~~~~~~~람다식(Lambda)식 표기 방법~~~~~~~~~~~");

		InterAreaFuntional_3 obj = new InterAreaFuntional_3() {
			@Override
		...
		//1. 람다식(Lambda)식 표기 기본방법
		
		InterAreaFuntional_3 objA = (String state) -> {System.out.println("웃음소리1 : "+state);};
		objA.smile("헤헤");
		
		//2. 실행명령문이 1개라면 {}를 생략할 수 있다
		
		InterAreaFuntional_3 objB = (String state) -> System.out.println("웃음소리2 : "+state);
		
		objB.smile("허허");

		//3. 파라미터 타입은 생략할 수 있다
		
		InterAreaFuntional_3 objC = (state) -> System.out.println("웃음소리3 : "+state);
		
		objC.smile("호호호");

		//4. 파라미터의 개수가 1개일때 파라미터를 감싸는 괄호() 생략할 수 있다
		
		InterAreaFuntional_3 objD = state -> System.out.println("웃음소리4 : "+state);
		
		objD.smile("히히히");
	
		//5. 파라미터가 없는 람다식은 무조건 () 괄호를 기재해야 한다.
		
		InterAreaFuntional_4 objE = () -> System.out.println("안녕하세요~~~");
		objE.write();
		
		//6. 파라미터가 2개 이상일 때는 무조건 () 괄호를 기재해야 한다.
		
		InterAreaFuntional_1 objF = (x,y) -> {return x+y;};
		double db1 = objF.area(10.5, 20.5);
		System.out.println(db1);
	
		//7. return이 있을 경우에는 {} 생략할 수 없다.
		
		// InterAreaFuntional_1 objG = (x,y) -> return x+y;		//오류
		InterAreaFuntional_1 objG = (x,y) -> {return x-y;};
		double db2 = objG.area(30.5, 20.5);
		System.out.println(db2);
	
		//8. return이 있는 단일 코드인 경우에는 {} 및 return을 생략
		
		// InterAreaFuntional_1 objG = (x,y) -> return x+y;		//오류
		InterAreaFuntional_1 objH = (x,y) -> x+y;
