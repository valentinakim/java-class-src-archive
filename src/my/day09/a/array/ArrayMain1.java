...
public class ArrayMain1 {
	...
	public static void main(String[] args) {
		...
		/*
		 	...
		 	위의 숫자는 배여의 인덱스(index)를 가리키는 번호로써 0부터 시작하여 1씩 증가한다.
		 	...
		 	정수는 0, 실수는 0.0 char는 ' ', String을 포함한 객체는 null
		 	
		 */
		
		// 배열의 크기(길이)는 배열명.length로 알아온다.
		...
		double avg = Math.round((double)sum/subjectArr.length*10)/10.0; //총합 나누기 배열의 크기(과목의 개수)
		...
		/*
 		1. 배열 선언 과
		2. 선언되어진 배열을 메모리에 할당을 해준다. 를
		각각 따로 하지 않고 동시에 할 수 있다.
		...
		avg = Math.round((double)sum/subjectArr2.length*10)/10.0; //총합 나누기 배열의 크기(과목의 개수)
		...
		avg = Math.round((double)sum/subjectArr3.length*10)/10.0; //총합 나누기 배열의 크기(과목의 개수)
		...
		avg = Math.round((double)sum/subjectArr4.length*10)/10.0; //총합 나누기 배열의 크기(과목의 개수)
		...
		int total=0;
		for(int subject :subjectArr4) {	//배열의 길이만큼 반복한다
			// 반복하면서 배열방에 들어있던 값을 차례차례 int subject 변수에 넣어준다
